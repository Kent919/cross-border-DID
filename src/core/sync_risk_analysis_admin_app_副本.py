# src/core/sync_risk_analysis_admin_app.py
...
def build_bayesian_network(data):
    ...
    hc = HillClimbSearch(data, scoring_method=BicScore(data))  # 使用 BIC 评分进行结构学习
    best_model = hc.estimate()  # 获取最优模型
...
def calculate_risk_probability(row, cpd_dict, params):
    ...
    for node in cpd_dict:  # 遍历条件概率分布
        if node in row and row[node] < len(cpd_dict[node]):  # 防止索引越界
            prob = cpd_dict[node][row[node]]  # 获取概率值
            node_prob *= prob  # 累乘概率
